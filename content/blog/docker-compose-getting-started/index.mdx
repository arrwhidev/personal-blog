...
[Docker Compose](https://docs.docker.com/compose/) is a tool for defining and managing applications which comprise of multiple Docker containers.

First, let's create some working directories;
...
Next let's create two simple applications; an API and a web server. I've chosen to write the API in [Go](https://golang.org/) and the web server in [Node](https://nodejs.org/en/) to keep things interesting. All the code is provided so don't worry if you are unfamiliar with these are technologies.
...
In here we will define some tructures and hardcode some data. The `main()` function simply listens to the root path on port 80 and returns a JSON response containing all the data in response to all requests.

I hardcoded the data in effort to keep things minimal. By the end of this post you will have the knowledge to enable you to move that data into a real database, also managed by Docker Compose.
...
When you application becomes more complex, fetching dependencies etc, this file will need to reflect those changes.

Ok, let's try out our container. I'm not going to explain most of the flags passed to these commands, please refer to the Docker documentation. I'll just say that we are building a container and running it. Finally, we are mapping port 80->8091 so the curl command has been updated to reflect this.
...
Finally, we have a new concept, `depends_on`. This tells Docker Compose that `films-web` depends on `films-api`. This is especially handy when you have many services and databases. Internally this just ensures that the API is started before the web service. It does **NOT** wait for the API to be 'ready', to handle this please read the [documentation](https://docs.docker.com/compose/startup-order/).
...
Now we have a single entry to spin up our containers.

If you want to run it in detached mode use the `-d` flag. Later you can stop it by running `docker-compose down`.
...
I've implemented a naive `fetch()` function using than pull in a dependency to do this - again keeping things simple. You'll see that we can now refer to the API using the service name defined in the Docker Compose file as the hostname `fetch('http://films-api')`.
